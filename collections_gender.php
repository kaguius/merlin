<?php
	...
	if($adminstatus == 4){
		...
		$page_title = "Collection Rates per Marital Status Report";
		include_once('includes/header.php');
		$filter_clerk = 0;
		if (!empty($_GET)){	
		...
		<div id="page">
			<div id="content">
				<div class="post">
					...
					<?php
						if($branch == ''){
							$sql = mysql_query("select distinct users.marital, sum(loan_total_interest)amount from loan_application inner join users on loan_application.customer_id = users.id where loan_application.loan_date between '$filter_start_date' and '$filter_end_date' group by users.marital");
						}
						else{
							$sql = mysql_query("select distinct users.marital, sum(loan_total_interest)amount from loan_application inner join users on loan_application.customer_id = users.id where loan_application.loan_date between '$filter_start_date' and '$filter_end_date' and loan_application.customer_station = '$branch' group by users.marital");
						}
						 ...
						 while ($row = mysql_fetch_array($sql))
						 {
							...
							while($row = mysql_fetch_array($sql2)) {
								$marital_name = $row['marital'];
							}
							
							$sql2 = mysql_query("select loan_code from loan_application inner join users on users.id = loan_application.customer_id where users.marital = '$marital' and loan_due_date between '$filter_start_date' and '$filter_end_date'");	
							while($row = mysql_fetch_array($sql2)) {
							...
							echo "<td valign='top'>$marital_name</td>";
