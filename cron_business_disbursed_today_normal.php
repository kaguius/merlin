<?php
	...
	if($adminstatus == 4){
		...
		$filter_start_date = '2017-01-15';
		$filter_end_date = '2017-02-16';
		$filter_repayments_date = '2017-02-16';
					...
					<?php
						...
						$sql = mysql_query("select loan_id, customer_id, customer_station, loan_date, loan_due_date, loan_mobile, initiation_fee, loan_amount, loan_extension, loan_interest, loan_late_interest, waiver, admin_fee, appointment_fee, early_settlement, early_settlement_surplus, fix, joining_fee, loan_total_interest, loan_code, loan_mpesa_code, loan_failure_status, loan_status from loan_application where loan_date between '$filter_start_date' and '$filter_end_date' and loan_failure_status = '0' and loan_status NOT IN ('11', '12', '14') order by loan_date asc");
						 ...
						 while ($row = mysql_fetch_array($sql))
						 {
							...
							$sql2 = mysql_query("select sum(loan_rep_amount)repayments from loan_repayments where loan_rep_code = '$loan_code' and loan_rep_date between '$filter_start_date' and '$filter_repayments_date' group by loan_rep_code");
							while ($row = mysql_fetch_array($sql2)) {
							...
							$sql2 = mysql_query("select loan_rep_date from loan_repayments where loan_rep_code = '$loan_code' and loan_rep_date between '$filter_start_date' and '$filter_repayments_date' order by loan_rep_date desc limit 1");
							while ($row = mysql_fetch_array($sql2)) {
							...
							echo "</tr>";
							$sql3 = mysql_query("select loan_rep_date, loan_rep_amount from loan_repayments where loan_rep_date between '$filter_start_date' and '$filter_repayments_date' and loan_rep_code = '$loan_code' order by loan_rep_date asc");
