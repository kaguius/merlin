...
?>
	...
	$sql = mysql_query("select loan_id, loan_date, loan_due_date, loan_mobile, customer_id, customer_station, loan_status, loan_mpesa_code, loan_code, initiation_fee, loan_amount, loan_extension, loan_interest, loan_late_interest, waiver, admin_fee, appointment_fee, early_settlement, early_settlement_surplus, fix, early_settlement_surplus, joining_fee, loan_total_interest from loan_application where loan_status != '13' and loan_status != '12' and loan_status != '11' and loan_status != '10' order by customer_id asc");
	while ($row = mysql_fetch_array($sql))
	{
		...
		$fees = $initiation_fee + $loan_extension + $loan_interest + $loan_late_interest + $waiver + $admin_fee + $appointment_fee + $early_settlement + $early_settlement_surplus + $fix + $joining_fee;
