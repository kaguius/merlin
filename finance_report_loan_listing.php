<?php
	...
	if($adminstatus == 4){
		...
		$page_title = "Finance Report: Loans Listing Report";
		...
		<div id="page">
			<div id="content">
				<div class="post">
					...
					<table width="100%" border="0" cellspacing="2" cellpadding="2" class="display" id="exmple2">
					...
					<?php
						...
						$sql = mysql_query("select loan_id, customer_id, customer_station, loan_date, loan_due_date, loan_mobile, initiation_fee, loan_amount, loan_extension, loan_interest, loan_late_interest, waiver, admin_fee, appointment_fee, early_settlement, early_settlement_surplus, fix, joining_fee, loan_total_interest, loan_code, loan_mpesa_code, loan_failure_status, loan_status from loan_application where loan_date between '$filter_start_date' and '$filter_end_date' and loan_failure_status = '0' and loan_status NOT IN ('11', '12', '14') order by loan_date asc");
						 ...
						 while ($row = mysql_fetch_array($sql))
						 {
							...
							//echo "select sum(loan_rep_amount)repayments from loan_repayments where loan_rep_code = '$loan_code' and loan_rep_date between '$filter_start_date' and '$filter_end_date' group by loan_rep_code";
							while ($row = mysql_fetch_array($sql2)) {
							...
							$sql2 = mysql_query("select loan_rep_date from loan_repayments where loan_rep_code = '$loan_code' and loan_rep_date between '$filter_start_date' and '$filter_end_date' order by loan_rep_date desc limit 1");
							while ($row = mysql_fetch_array($sql2)) {
								$loan_rep_date = $row['loan_rep_date'];
							}
							
							if($loan_status_name = 'Settled'){
								if($loan_rep_date == ""){
									$end = strtotime($loan_due_date);
								}
								else{
									$end = strtotime($loan_rep_date);
								}
								$start = strtotime($loan_date);
								$datediff = $end - $start;
								$days_with_loan = floor($datediff / (60 * 60 * 24));
								if($days_with_loan == 31) { 
									$days_with_loan = 30; 
								}
								$days_with_loan = 30; 
								$interest_comp = ((0.45 / 100) * $days_with_loan) * $loan_amount;
								$admin_comp = ((0.22 / 100) * $days_with_loan) * $loan_amount;
								$insurance_comp = ((0.33 / 100) * $days_with_loan) * $loan_amount;
							}
							...
							echo "<td valign='top' align='right'>".number_format($interest_comp, 2)."</td>";
							echo "<td valign='top' align='right'>".number_format($admin_comp, 2)."</td>";
							echo "<td valign='top' align='right'>".number_format($insurance_comp, 2)."</td>";
