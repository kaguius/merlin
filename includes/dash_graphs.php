    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var data = google.visualization.arrayToDataTable([
          ['Loan Status', 'Totals'],
          <?php
          	$result_tender = mysql_query("select distinct call_outcome, count(id)counts from promise_to_pay where call_outcome != '0' and UID = '$userid' group by call_outcome");
			while ($row = mysql_fetch_array($result_tender))
			{
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ...
          <?php
          	$result = mysql_query("select distinct lr.current_collector, sum(lr.loan_rep_amount)loan_rep_amount from loan_repayments lr inner join loan_application l on l.loan_code = lr.loan_rep_code where extract(month from loan_rep_date) = '$filter_month' and extract(year from loan_rep_date) = '$filter_year' and lr.current_collector != '0' and vintage != '' group by lr.current_collector order by loan_rep_amount desc");
			while ($row = mysql_fetch_array($result))
			{
