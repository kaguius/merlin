<?php
	...
	if($adminstatus == 4){
		...
		$sql = mysql_query("select first_name, last_name, mobile_no, dis_phone, affordability, loan_officer, collections_officer, stations, limit_loan_amount from users where id = '$user_id'", $dbh1);
		...
		if ($mode=='edit'){
			$sql = mysql_query("select loan_id, loan_date, loan_term, loan_due_date, customer_id, loan_mobile, loan_amount, loan_total_interest, loan_status, loan_code, loan_mpesa_code, waiver, loan_disbursed, loan_failure_status, customer_state, loan_officer, collections_officer, comment, UID, late_status, collections_agent, admin_fee, appointment_fee, early_settlement, early_settlement_surplus, fix, joining_fee, loan_late_interest, initiation_fee, loan_interest from loan_application where loan_id = '$loan_id'", $dbh1);
			while ($row = mysql_fetch_array($sql))
			{					
				$loan_id = $row['loan_id'];
						...
						<?php if($regression_days > '0'){ ?>
							<td valign='top' colspan="3">
								<select name='loan_amount' id='loan_amount'>
									...
									<option value=''> </option>	
									<?php
									for ($x = 5000; $x <= $loan_amount; $x = $x+5000) {
								...
								<?php
									...
									if($affordability >= 5000){
										if($loan_count == 0){
											...
											//	for ($x = 5000; $x <= $limit_loan_amount; $x = $x+5000) {
											//		echo "<option value='$x'>".number_format($x, 2)."</option>"; 
											//	} 
											//}
											//else{
						...
						<td valign='top' width="35%">
							<select name='collections_agent' id='collections_agent'>
								...
								<?php
									...
									//echo "<option value=''>" "</option>";
									if($station == '3'){ 
										$sql2 = mysql_query("select user_profiles.id, first_name, last_name, stations.stations from user_profiles inner join stations on stations.id = user_profiles.station where title = '7'", $dbh1);
									}
									//else{										
...
<?php
	if (!empty($_POST)) {
		...
		$sql = mysql_query("select count(loan_id)loan_count from loan_application where customer_id = '$user_id'", $dbh1);
		while ($row = mysql_fetch_array($sql))
		{
			$loan_count = $row['loan_count'];
			if($loan_count == ""){
		...
		if($page_status == 'edit'){
			if($loan_status == '5'){
				...
				$sql3="
				update loan_application set loan_mpesa_code='$loan_mpesa_code', customer_state = '$customer_state', loan_officer='$loan_officer', collections_officer='$collections_officer', comment ='$comment', loan_status='$loan_status', loan_failure_status='$loan_failure_status', admin_fee = '$admin_fee_from_form', loan_status = '$loan_status', UID='$userid', loan_interest = '$loan_interest_from_form', early_settlement='$early_settlement_from_form',  early_settlement_surplus = '$early_settlement_surplus_from_form', loan_late_interest = '$loan_late_interest_from_form', loan_total_interest = '$loan_total_interest', collections_agent='$collections_agent' WHERE loan_id  = '$loan_id'";
				...
				if($exists_loan_mpesa_code == $loan_mpesa_code && $loan_status != '11' && $loan_status != '12' && $loan_status != '14'){
					...
					$sql3="update loan_application set initiation_fee = '$initiation_fee_from_form', loan_amount = '$loan_amount_from_form', loan_interest = '$loan_interest_from_form', loan_late_interest = '$loan_late_interest_from_form', admin_fee = '$admin_fee_from_form', appointment_fee = '$appointment_fee_from_form', early_settlement = '$early_settlement_from_form', early_settlement_surplus = '$early_settlement_surplus_from_form', fix = '$fix_from_form', joining_fee = '$joining_fee_from_form', loan_total_interest = '$loan_total_interest', loan_status = '$loan_status', loan_mpesa_code = '$loan_mpesa_code', loan_officer = '$loan_officer', collections_officer = '$collections_officer', collections_agent='$collections_agent', customer_state = '$customer_state', comment = '$comment', UID = '$UID' WHERE loan_id  = '$loan_id'";
				...
				$sql = mysql_query("select distinct loan_code, msg from loan_application where loan_code = '$loan_code' and loan_status = '10' order by loan_code desc limit 1", $dbh1);
				while ($row = mysql_fetch_array($sql))
				{
				...
				if($msg == '0'){				
					$message_text = "Thanks for choosing UPIA. We have disbursed ".number_format($loan_amount_from_form, 0).". Your due date is ".$loan_due_date." and the amount due is ".number_format($loan_total_interest, 0).". Your loan ref: ".$loan_code.". Kopa UPIA, kuza biashara.";
					
					if($dis_phone == $mobile_no){
						$sql5="INSERT INTO out_msg_logs (loan_code, customer_id, mobile_no, msg_text, status, new, transactiontime) 
						VALUES('$loan_code_latest', '$user_id', '$dis_phone', '$message_text', '0', '1', '$transactiontime')";
					}
					else{
						$sql5="INSERT INTO out_msg_logs (loan_code, customer_id, mobile_no, msg_text, status, new, transactiontime) 
						VALUES('$loan_code_latest', '$user_id', '$dis_phone', '$message_text', '0', '1', '$transactiontime')";
						...
						VALUES('$loan_code_latest', '$user_id', '$mobile_no', '$message_text', '0', '1', '$transactiontime')";			
					}
			
					$sql6="update loan_application set msg='1' WHERE loan_id  = '$loan_id'";
					
				}
				if($mobile_carrier == 'Safaricom'){
					$sql8="INSERT INTO mobile_money_requests (loan_code, msisdn, amount, carrier, new, transactiontime) 
					VALUES('$loan_code_latest', '$dis_phone', '$loan_amount_from_form', '1', '1', '$transactiontime')";	
				}
			...
			VALUES('$loan_date', '$loan_term', '$loan_due_date', '$user_id', '$dis_phone', '$initiation_fee', '$loan_amount', '$loan_interest', '$loan_total_interest', '10', '$loan_code_latest', '$loan_mpesa_code', '$loan_disbursed', '$loan_failure_status', '$loan_officer', '$collections_officer', '$comment', '$userid', '$customer_station', '1', '$customer_state')";
