...
if ($adminstatus == 4) {
    ...
    $sql = mysql_query("select freeze from stations where id = '$station'", $dbh1);
    while ($row = mysql_fetch_array($sql)) {
        $freeze = $row['freeze'];
    }
    ...
    if ($mode == 'edit') {
        $sql = mysql_query("select loan_id, loan_date, loan_term, loan_due_date, customer_id, loan_mobile, loan_amount, loan_total_interest, loan_status, loan_code, loan_mpesa_code, waiver, loan_disbursed, loan_failure_status, customer_state, loan_officer, collections_officer, comment, UID, late_status, collections_agent, field_agent, current_collector, admin_fee, appointment_fee, early_settlement, early_settlement_surplus, fix, joining_fee, loan_late_interest, initiation_fee, loan_interest, loan_failure_status from loan_application where loan_id = '$loan_id'", $dbh1);
        while ($row = mysql_fetch_array($sql)) {
        $loan_id = $row['loan_id'];
        ...
        $sql2 = mysql_query("select id, first_name, last_name from user_profiles where id = '$current_collector'", $dbh1);
        while ($row = mysql_fetch_array($sql2)) {
        //$collections_agent_id = $row['id'];
            $current_collector_id = $row['id'];
            $first_name = $row['first_name'];
            $last_name = $row['last_name'];
            //$collections_agent = $first_name . " " . $last_name;
            $current_collector_name = $first_name . " " . $last_name;
        }
    ...
    <?php if ($freeze == '0') { ?>
       <table width="60%">
        <tr bgcolor="red">
         <td><font color="white" size="2">&nbsp;&nbsp;The unit/ satellite is on Freeze. Please contact management.</td>
        </tr>
       </table>
    <?php } ?>
    ...
    <?php } ?>
         <tr>
          ...
          <td valign='top' >Loan Amount *</td>
            <?php
            //Determine the loan loan value that was successfully paid
            $sql = mysql_query("select max(loan_amount)loan_amount from loan_application where customer_id = '$user_id' and loan_status = '13'", $dbh1);
            while ($row = mysql_fetch_array($sql)) {
                $current_loan_amount = $row['loan_amount'];
                //$initiation_fee_new = $row['initiation_fee'];
            }
            
            //Determine if this is a new customer by checking if they have been charged the initiation fee before
            $sql = mysql_query("select sum(initiation_fee)initiation_fee from loan_application where customer_id = '$user_id' and loan_status != '12'", $dbh1);
            while ($row = mysql_fetch_array($sql)) {
                $initiation_fee_new = $row['initiation_fee'];
            }
            if($initiation_fee_new > '0'){
                $highest = $current_loan_amount + 5000;
                if($highest > $affordability){
                    $highest = $affordability;
                }
            }
            else{
                $highest_value = $affordability - 2500;
                if($highest_value > 20000){
                    $highest = 20000;
                }
                else{
          ...
          <td valign='top' colspan="3">
               <input title="Enter Loan Amount" value="<?php echo $previous_loan_amount ?>" id="loan_amount" name="loan_amount" type="text" maxlength="100" readonly class="main_input" size="35" />
          </td>
          <?php } else{ ?>
          <td valign='top' colspan="3">
               <select name='loan_amount' id='loan_amount'>
                    ...
                    if ($limit_loan_amount > '0') {
                        if($limit_loan_amount > $affordability){
                            $limit_loan_amount = $affordability;
                            $sql2 = mysql_query("select id, loan_band from loan_bands where loan_band <= '$limit_loan_amount' order by id asc", $dbh1);
                            while ($row = mysql_fetch_array($sql2)) {
                        ...
                        $sql2 = mysql_query("select id, loan_band from loan_bands where loan_band <= '$highest' order by id asc", $dbh1);
                        while ($row = mysql_fetch_array($sql2)) {
                            $loan_band = $row['loan_band'];
                            echo "<option value='$loan_band'>" . number_format($loan_band, 2) . "</option>";
                            //echo "<option value='$x'>" . number_format($x, 2) . "</option>";
         ...
         <tr >
          ...
          <td valign='top' width="35%">
           <select name='loan_status' id='loan_status'>
            ...
            if ($title == '3') {
                $sql2 = mysql_query("select id, status from customer_status where id = '16' or id = '2' order by status asc", $dbh1);
            }
            else if ($title == '11') {
                $sql2 = mysql_query("select id, status from customer_status where id = '16' or id = '2' order by status asc", $dbh1);
            }
            else if ($title == '8') {
                $sql2 = mysql_query("select id, status from customer_status where id = '10' or id = '16' order by status asc", $dbh1);
            }
            else {
          ...
          <td valign='top' width="35%">
           <select name='current_collector' id='current_collector'>
          <?php
          if ($mode == 'edit') {
          ?>
            <option value="<?php echo $current_collector_id ?>"><?php echo $current_collector_name ?></option>
            <option value=''> </option>	
              <?php
              }
              else {
            ...
            <option value=''> </option>
               ...
               //echo "<option value=''>" "</option>";
               if ($station == '3') {
               $sql2 = mysql_query("select user_profiles.id, first_name, last_name, stations.stations from user_profiles inner join stations on stations.id = user_profiles.station where title in( '7','9','10') and user_status = '1'", $dbh1);
               }
               //else{										
           ...
           <input value="<?php echo $current_collector_id ?>" id="old_current_collector" name="old_current_collector" type="hidden" />
          </td>
            <?php } ?>
            </tr>
          <?php if ($station == '3' || $station == '4') { ?>
          <td valign='top' width="15%">Customer State (BFC/BLC/EDC/FA)</td>
          <td valign='top' width="35%" colspan="3">
           ...
           ?>
            ...
            echo "<option value='EDC'>EDC</option>";
    ...
    $current_collector = $_POST['current_collector'];
    $old_current_collector = $_POST['old_current_collector'];
    ...
    //echo $$current_collector."<br />";

    $sql_collector = mysql_query("select title where id = '$current_collector'", $dbh1);
    while ($row = mysql_fetch_array($sql)) {

    $collector_title = $row['title'];

    if ($collector_title == "7") {
    $collections_agent = $current_collector;
    }
    elseif ($collector_title == "9") {
    $edc = $current_collector;
    }
    elseif ($collector_title == "10") {
    $field_agent = $current_collector;
    }
    }
    ...
    $loan_term = $days;
    //Two month loan from 14h Nov 2016 to 2nd Dec 2016
	//Only repeat customers from period above
	//Compute
	
    if ($current_date >= '2016-11-14' && $current_date <= '2016-12-02'){
    	$loan_term = 60;
    ...
    $sql3 = "
    update loan_application set loan_mpesa_code='$loan_mpesa_code', customer_state = '$customer_state', loan_officer='$loan_officer', collections_officer='$collections_officer', comment ='$comment', loan_status='$loan_status', loan_failure_status='$loan_failure_status', admin_fee = '$admin_fee_from_form', loan_status = '$loan_status', UID='$userid', loan_interest = '$loan_interest_from_form', early_settlement='$early_settlement_from_form',  early_settlement_surplus = '$early_settlement_surplus_from_form', appointment_fee = '$appointment_fee_from_form', loan_late_interest = '$loan_late_interest_from_form', joining_fee = '$joining_fee_from_form', fix = '$fix_from_form', loan_total_interest = '$loan_total_interest', collections_agent='$current_collector', field_agent= '$field_agent', edc = '$edc', current_collector='$current_collector' WHERE loan_id  = '$loan_id'";

    $sql10 = "update users set customer_state ='$customer_state', collections_agent='$current_collector' WHERE id  = '$user_id'";
    //echo $sql10."<br />";
    $result = mysql_query($sql10, $dbh1);

        if ($customer_state == 'BFC') {
            $sql11 = "update loan_application set loan_status = '5', late_status = '2' WHERE loan_id = '$loan_id'";
        }
    }
    else {
    ...
    $sql = mysql_query("select distinct loan_code, customer_station, msg from loan_application where loan_code = '$loan_code' and loan_status = '16' order by loan_code desc limit 1", $dbh1);
    while ($row = mysql_fetch_array($sql)) {
    ...
    if ($loan_failure_status_form == '1' && $loan_status == '2' && $station != '16') {
        if ($mobile_carrier == 'Safaricom') {
            ...
            VALUES('$loan_code', '$dis_phone', '$loan_amount_from_form', '1', '1', '$customer_station', '$transactiontime')";
        }
        else if ($mobile_carrier == 'Airtel') {
            $sql9 = "INSERT INTO mobile_money_requests (loan_code, msisdn, amount, carrier, new, customer_station, transactiontime) 
            VALUES('$loan_code', '$dis_phone', '$loan_amount_from_form', '2', '1', '$customer_station', '$transactiontime')";
        }
    }
    ...
    $sql43 = "INSERT INTO loan_application (loan_date, loan_term, loan_due_date, customer_id, loan_mobile, initiation_fee, loan_amount, loan_interest, loan_total_interest, loan_status, loan_code, loan_mpesa_code, loan_disbursed, loan_failure_status, loan_officer, collections_officer, comment, UID, customer_station, loan_creation, customer_state) VALUES('$loan_date', '$loan_term', '$loan_due_date', '$user_id', '$dis_phone', '$initiation_fee', '$loan_amount', '$loan_interest', '$loan_total_interest', '10', '$loan_code_latest', '$loan_mpesa_code', '$loan_disbursed', '1', '$loan_officer', '$collections_officer', '$comment', '$userid', '$customer_station', '1', '$customer_state')";

    $sql53 = "INSERT INTO call_center (loan_date, loan_term, loan_due_date, customer_id, loan_mobile, initiation_fee, loan_amount, loan_interest, loan_total_interest, loan_status, loan_code, loan_mpesa_code, loan_disbursed, loan_failure_status, loan_officer, collections_officer, comment, UID, customer_station, loan_creation, customer_state) VALUES('$loan_date', '$loan_term', '$loan_due_date', '$user_id', '$dis_phone', '$initiation_fee', '$loan_amount', '$loan_interest', '$loan_total_interest', '10', '$loan_code_latest', '$loan_mpesa_code', '$loan_disbursed', '1', '$loan_officer', '$collections_officer', '$comment', '$userid', '$customer_station', '1', '$customer_state')";
    }
    ...
    if ($freeze != '0'){
        ...
        $result = mysql_query($sql43, $dbh1);
        $result = mysql_query($sql53, $dbh1);
    }		
