...
if ($adminstatus == 4) {
    ...
    if ($dis_phone_prefix == 25470 || $dis_phone_prefix == 25471 || $dis_phone_prefix == 25472) {
    ...
    if ($mode == 'edit') {
        $sql = mysql_query("select loan_id, loan_date, loan_term, loan_due_date, customer_id, loan_mobile, loan_amount, loan_total_interest, loan_status, loan_code, loan_mpesa_code, waiver, loan_disbursed, loan_failure_status, customer_state, loan_officer, collections_officer, comment, UID, late_status, collections_agent, current_collector, admin_fee, appointment_fee, early_settlement, early_settlement_surplus, fix, joining_fee, loan_late_interest, initiation_fee, loan_interest, loan_failure_status from loan_application where loan_id = '$loan_id'", $dbh1);
        while ($row = mysql_fetch_array($sql)) {
            $loan_id = $row['loan_id'];
    ...
    if (!empty($_POST)) {
        ...
        if ($page_status == 'edit') {
            if ($loan_status == '5') {
                ...
                // Update users table
                $sql_update_users_table = "update users set customer_state ='$customer_state', collections_agent='$collections_agent' WHERE id  = '$user_id'";
                $result = mysql_query($sql_update_users_table, $dbh1);

                // Check if collection agent is a field agent or collections agent
                $sql_get_agent_title = mysql_query("select title from user_profiles where id = '$collections_agent'", $dbh1);
                while ($row = mysql_fetch_array($sql_get_agent_title)) {
                    $title = $row['title'];
                }
                
                // Get current collector & field agent
                $sql_get_current_collector = mysql_query("select current_collector, field_agent from loan_application where loan_code = '$loan_code'", $dbh1);
                while ($row = mysql_fetch_array($sql_get_current_collector)) {
                    $current_collector = $row['current_collector'];
                    $old_field_agent = $row['field_agent'];
                }

                if ($title == '7') {
                    // This is a collections agent
                    $sql_update_loan = "update loan_application set loan_mpesa_code='$loan_mpesa_code', customer_state = '$customer_state', loan_officer='$loan_officer', collections_officer='$collections_officer', comment ='$comment', loan_status='$loan_status', loan_failure_status='$loan_failure_status', admin_fee = '$admin_fee_from_form', loan_status = '$loan_status', UID='$userid', loan_interest = '$loan_interest_from_form', early_settlement='$early_settlement_from_form',  early_settlement_surplus = '$early_settlement_surplus_from_form', appointment_fee = '$appointment_fee_from_form', loan_late_interest = '$loan_late_interest_from_form', joining_fee = '$joining_fee_from_form', fix = '$fix_from_form', loan_total_interest = '$loan_total_interest', collections_agent = '$collections_agent', current_collector='$collections_agent', arrears_assigned = '$transactiontime' WHERE loan_id  = '$loan_id'";
                    $sql_update_users = "update users set collections_agent='$collections_agent' WHERE id  = '$user_id'";
                    $sql_record_agent_change = "insert into change_log(UID, table_name, customer_id, variable, loan_code, old_value, new_value, transactiontime)values('$userid', 'loan_application', '$user_id', 'collections_agent', '$loan_code', '$old_collections_agent', '$collections_agent', '$transactiontime')";
                    $sql_record_current_collector_change = "insert into change_log(UID, table_name, customer_id, variable, loan_code, old_value, new_value, transactiontime)values('$userid', 'loan_application', '$user_id', 'current_collector', '$loan_code', '$current_collector', '$collections_agent', '$transactiontime')";

                    $result_update_loan = mysql_query($sql_update_loan, $dbh1);
                    $result_update_users = mysql_query($sql_update_users, $dbh1);
                    $result_record_agent_change = mysql_query($sql_record_agent_change, $dbh1);
                    $result_record_current_collector_change = mysql_query($sql_record_current_collector_change, $dbh1);
                } else if ($title == '10') {
                    // This is a field agent
                    $sql_update_loan = "update loan_application set loan_mpesa_code='$loan_mpesa_code', customer_state = '$customer_state', loan_officer='$loan_officer', collections_officer='$collections_officer', comment ='$comment', loan_status='$loan_status', loan_failure_status='$loan_failure_status', admin_fee = '$admin_fee_from_form', loan_status = '$loan_status', UID='$userid', loan_interest = '$loan_interest_from_form', early_settlement='$early_settlement_from_form',  early_settlement_surplus = '$early_settlement_surplus_from_form', appointment_fee = '$appointment_fee_from_form', loan_late_interest = '$loan_late_interest_from_form', joining_fee = '$joining_fee_from_form', fix = '$fix_from_form', loan_total_interest = '$loan_total_interest', field_agent = '$collections_agent', current_collector='$collections_agent', assigned_field_agent = '$transactiontime' WHERE loan_id  = '$loan_id'";
                    $sql_record_agent_change = "insert into change_log(UID, table_name, customer_id, variable, loan_code, old_value, new_value, transactiontime)values('$userid', 'loan_application', '$user_id', 'field_agent', '$loan_code', '$old_field_agent', '$collections_agent', '$transactiontime')";
                    $sql_record_current_collector_change = "insert into change_log(UID, table_name, customer_id, variable, loan_code, old_value, new_value, transactiontime)values('$userid', 'loan_application', '$user_id', 'current_collector', '$loan_code', '$current_collector', '$collections_agent', '$transactiontime')";
                    ...
                    // Collectons agent not set
                    $sql_update_loan = "update loan_application set loan_mpesa_code='$loan_mpesa_code', customer_state = '$customer_state', loan_officer='$loan_officer', collections_officer='$collections_officer', comment ='$comment', loan_status='$loan_status', loan_failure_status='$loan_failure_status', admin_fee = '$admin_fee_from_form', loan_status = '$loan_status', UID='$userid', loan_interest = '$loan_interest_from_form', early_settlement='$early_settlement_from_form',  early_settlement_surplus = '$early_settlement_surplus_from_form', appointment_fee = '$appointment_fee_from_form', loan_late_interest = '$loan_late_interest_from_form', joining_fee = '$joining_fee_from_form', fix = '$fix_from_form', loan_total_interest = '$loan_total_interest' WHERE loan_id  = '$loan_id'";

                    $result_update_loan = mysql_query($sql_update_loan, $dbh1);
                }

                if ($customer_state == 'BFC') {
                    $sql_update_loan_status = "update loan_application set loan_status = '5' WHERE loan_id = '$loan_id'";
                    $result = mysql_query($sql_update_loan_status, $dbh1);
                }
            } else {
                ...
                $exists_loan_mpesa_code = MD5(exists_loan_mpesa_code);
                $query = "loan_details.php?status=exists_loan_mpesa_code&exists_loan_mpesa_code=$exists_loan_mpesa_code&user_id=$user_id&loan_id=$loan_id&mode=edit";
                ?>
                <script type="text/javascript">
                    <!--
                //			document.location = "<?php echo $query ?>";
                ...
                if ($title == '7') {
                    // This is a collections agent
                    $sql_update_loan = "update loan_application set initiation_fee = '$initiation_fee_from_form', loan_amount = '$loan_amount_from_form', loan_interest = '$loan_interest_from_form', loan_late_interest = '$loan_late_interest_from_form', admin_fee = '$admin_fee_from_form', appointment_fee = '$appointment_fee_from_form', early_settlement = '$early_settlement_from_form', early_settlement_surplus = '$early_settlement_surplus_from_form', fix = '$fix_from_form', joining_fee = '$joining_fee_from_form', appointment_fee = '$appointment_fee_from_form', fix = '$fix_from_form', loan_total_interest = '$loan_total_interest', loan_status = '$loan_status', loan_mpesa_code = '$loan_mpesa_code', loan_officer = '$loan_officer', collections_officer = '$collections_officer', customer_state = '$customer_state', comment = '$comment', UID = '$UID', collections_agent = '$collections_agent', current_collector='$collections_agent', arrears_assigned =  WHERE loan_id  = '$loan_id'";
                    $sql_update_users = "update users set collections_agent='$collections_agent' WHERE id  = '$user_id'";
                    $sql_record_agent_change = "insert into change_log(UID, table_name, customer_id, variable, loan_code, old_value, new_value, transactiontime)values('$userid', 'loan_application', '$user_id', 'collections_agent', '$loan_code', '$old_collections_agent', '$collections_agent', '$transactiontime')";
                    $sql_record_current_collector_change_change = "insert into change_log(UID, table_name, customer_id, variable, loan_code, old_value, new_value, transactiontime)values('$userid', 'loan_application', '$user_id', 'current_collector', '$loan_code', '$old_collections_agent', '$collections_agent', '$transactiontime')";
                    ...
                    // This is a field agent
                    $sql_update_loan = "update loan_application set initiation_fee = '$initiation_fee_from_form', loan_amount = '$loan_amount_from_form', loan_interest = '$loan_interest_from_form', loan_late_interest = '$loan_late_interest_from_form', admin_fee = '$admin_fee_from_form', appointment_fee = '$appointment_fee_from_form', early_settlement = '$early_settlement_from_form', early_settlement_surplus = '$early_settlement_surplus_from_form', fix = '$fix_from_form', joining_fee = '$joining_fee_from_form', appointment_fee = '$appointment_fee_from_form', fix = '$fix_from_form', loan_total_interest = '$loan_total_interest', loan_status = '$loan_status', loan_mpesa_code = '$loan_mpesa_code', loan_officer = '$loan_officer', collections_officer = '$collections_officer', customer_state = '$customer_state', comment = '$comment', UID = '$UID', field_agent = '$collections_agent', current_collector='$collections_agent' WHERE loan_id  = '$loan_id'";
                    $sql_update_users = "update users set collections_agent='$collections_agent' WHERE id  = '$user_id'";
                    $sql_record_agent_change = "insert into change_log(UID, table_name, customer_id, variable, loan_code, old_value, new_value, transactiontime)values('$userid', 'loan_application', '$user_id', 'field_agent', '$loan_code', '$old_collections_agent', '$collections_agent', '$transactiontime')";
                    ...
                    // Collectons agent not set
                    $sql_update_loan = "update loan_application set initiation_fee = '$initiation_fee_from_form', loan_amount = '$loan_amount_from_form', loan_interest = '$loan_interest_from_form', loan_late_interest = '$loan_late_interest_from_form', admin_fee = '$admin_fee_from_form', appointment_fee = '$appointment_fee_from_form', early_settlement = '$early_settlement_from_form', early_settlement_surplus = '$early_settlement_surplus_from_form', fix = '$fix_from_form', joining_fee = '$joining_fee_from_form', appointment_fee = '$appointment_fee_from_form', fix = '$fix_from_form', loan_total_interest = '$loan_total_interest', loan_status = '$loan_status', loan_mpesa_code = '$loan_mpesa_code', loan_officer = '$loan_officer', collections_officer = '$collections_officer', customer_state = '$customer_state', comment = '$comment', UID = '$UID' WHERE loan_id  = '$loan_id'";

                    $result_update_loan = mysql_query($sql_update_loan, $dbh1);
                }

                if ($customer_state == 'BFC') {
                    $sql_update_bfc_status = "update loan_application set loan_status ='5' WHERE loan_code  = '$loan_code'";

                    $result_update_bfc_status = mysql_query($sql_update_bfc_status, $dbh1);
                }
            ...
            if ($loan_failure_status_form == '1' && $loan_status == '2' && $station != '16') {
                if ($mobile_carrier == 'Safaricom') {
                    ...
                    //if($mobile_carrier == 'Airtel'){
                    $sql8 = "INSERT INTO mobile_money_requests (loan_code, msisdn, amount, carrier, new, transactiontime) 
						VALUES('$loan_code_latest', '$dis_phone', '$loan_amount_from_form', '2', '1', '$transactiontime')";
                }
