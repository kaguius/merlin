				...
				<div class="post">
					...
					while ($row = mysql_fetch_array($sql))
					{
						...
						if($year >= '2015'){
							...
							$result = mysql_query("select sum(loan_late_interest)penalty from loan_application where extract(month from loan_date) = '$month' and extract(year from loan_date) = '$year'");
							while ($row = mysql_fetch_array($result))
							{
								$penalty = $row['penalty'];
