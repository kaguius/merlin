<?php
	...
	if($adminstatus == 4){
		...
		$page_title = "Income Report per Branch";
		include_once('includes/header.php');
		$filter_clerk = 0;
		if (!empty($_GET)){	
		...
		if ($filter_start_date != "" && $filter_end_date != ""){
			$sql2 = mysql_query("select stations from stations where id = '$stations' order by stations asc");
			while($row = mysql_fetch_array($sql2)) {
				$stations_name = $row['stations'];
			}
			$report_filter_start_date = date("d M, Y", strtotime($filter_start_date));
			$report_filter_end_date = date("d M, Y", strtotime($filter_end_date));
		...
		<div id="page">
			<div id="content">
				<div class="post">
					...
					<p><strong>Branch Name: <?php echo $stations_name ?><br />Report Date: <?php echo $report_filter_start_date ?> to <?php echo $report_filter_end_date ?></strong></p>
					...
					while ($row = mysql_fetch_array($sql))
					{
						...
						if($month > '10' && $year >= '2015'){
							...
							$result = mysql_query("select sum(loan_late_interest)penalty from loan_application where extract(month from loan_date) = '$month' and extract(year from loan_date) = '$year' and customer_station = '$stations'");
						...
						$result = mysql_query("select distinct loan_code from loan_application where extract(month from loan_date) = '$month' and extract(year from loan_date) = '$filter_year' and customer_station = '$stations' and loan_status != '12' and loan_status != '14' and loan_status != '11' and loan_status != '10'");
						while ($row = mysql_fetch_array($result))
						{
							$loan_code = $row['loan_code'];
							$sql3 = mysql_query("select sum(loan_rep_amount)repayments from loan_repayments where loan_rep_code = '$loan_code' and loan_rep_mpesa_code not like '%4G%' group by loan_rep_code");
			...
			<div id="content">
				<div class="post">
					...
					<table border="0" width="100%" cellspacing="2" cellpadding="2">
							...
							<tr>
								<td  valign="top">Select Branch:</td>
								<td> 
