<?php
	...
	if($adminstatus == 4){
		...
		include_once('includes/header.php');
		$filter_clerk = 0;
		$sql = mysql_query("select sum(paid_in)susp_acc_amount from suspence_accounts where resolved = '0'");
		...
		<div id="page">
			<div id="content">
				<div class="post">
					...
					<tbody>
					<?php
						 $sql = mysql_query("select id, receipt, date, details, status, paid_in, balance, balance_confirmed, trans_type, other_party_info, trans_party_details, resolved from suspence_accounts where resolved = '0' order by date desc");
						 $intcount = 0;
